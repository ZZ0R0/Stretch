// source_contribs.wgsl — compute source contributions per node
// 1 thread per node. Replaces CPU compute_source_contribs().
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>, @builtin(num_workgroups) nwg: vec3<u32>) {
    ...
    // Compute effective threshold (mirrors Node::effective_threshold in Rust)
    ...
    // Node must be active to contribute
    if (node.activation <= eff_threshold) {
        source_contribs[idx] = 0.0;
        return;
    }

    // Sign: excitatory → +1, inhibitory → -gain_inhibitory
